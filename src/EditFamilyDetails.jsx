...
const EditFamilyDetails = ({ member, onSave, onCancel }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // Make the API request to fetch employee profile data
        const response = await axios.get("http://192.168.0.119:8080/hrmsapplication/employee/getEmployeeProfile/HRMS1");
        
        // Assuming the response data contains the employee profile
        const data = response.data;
        
        // Set the form values with the data received from the API
        setFormValues(data);
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        // Send POST request with form values to the server
        const response = await axios.post("http://192.168.0.119:8080/hrmsapplication/employee/create", formValues);
        
        // Assuming the server returns the saved data as a response
        const data = response.data;
        
        // Log the response data to the console
        console.log("Form submitted successfully", data);
  
        // Update form values with the response data, or reset the form
        setFormValues({
          prefix: "",
          firstname: "",
          middlename: "",
